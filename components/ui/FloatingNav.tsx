...
export const FloatingNav = ({
  ...
  const handleLinkClick = (event: React.MouseEvent, link: string) => {
    ...
    setTimeout(() => {
      window.location.href = link; // Navigate after closing modal
    }, 50); // Add slight delay to allow modal to close
  };
    ...
    <>
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className={cn(
            "flex max-w-fit fixed top-5 inset-x-0 mx-auto border-[4px] rounded-full shadow-[0px_2px_3px_-1px_rgba(0,0,0,0.1),0px_1px_0px_0px_rgba(25,28,33,0.02),0px_0px_0px_1px_rgba(25,28,33,0.08)] z-[5000] px-10 py-5 items-center justify-center space-x-4 border-white/0.2 bg-black-100",
            className
          )}
        >
